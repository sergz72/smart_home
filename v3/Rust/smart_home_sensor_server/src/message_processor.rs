...
use smart_home_common::entities::{DeviceSensor, Message, MessageDateTime, Messages};
...
impl MessageProcessor for SensorMessageProcessor {
    fn process_message(&self, logger: &Logger, message: &Vec<u8>) -> Vec<u8> {
        ...
        match self.get_sensors(device_id) {
            Some(sensors) => 
                self.process_decrypted_message(logger, device_id, decrypted, sensors),
...
fn build_messages(logger: &Logger, decrypted: Vec<u8>, sensors: &HashMap<usize, DeviceSensor>,
    ...
    for (pidx, sensor) in sensors {
        ...
        let message = Messages{
            messages: vec![Message{value_type: sensor.value_type.clone(),
