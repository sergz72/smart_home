...
use smart_home_common::db::{create_database_from_configuration, Database};
use smart_home_common::entities::{Messages, SensorTimestamp};

pub struct Fetcher {
    config: Configuration,
    database: Box<dyn Database>,
    sensors_map: HashMap<String, SensorTimestamp>,
    aes: Aes128Gcm,
    time: u64,
    timeout: Duration
}
...
pub fn build_fetcher(config: Configuration) -> Result<Fetcher, Error> {
    ...
    Ok(Fetcher { config, database, sensors_map, aes, time, timeout })
}
