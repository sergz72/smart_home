...
fn main() -> Result<(), Error> {
    ...
    let db = create_database_from_configuration(&config.database_configuration)?;
    let key = read_key_file32(&config.key_file_name)?;
    let message_processor =
        build_message_processor(UserCommandProcessor::new(db, key)?, true, false, true)?;
    let udp_server =
        Box::leak(Box::new(BaseServer::new(true, config.port_number,
                                           message_processor.clone(), "udp_server".to_string())?));
    udp_server.start();
    Ok(())
}
