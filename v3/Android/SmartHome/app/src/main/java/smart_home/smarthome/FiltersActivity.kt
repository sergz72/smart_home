...
import smart_home.smarthome.service.DateOffset
import smart_home.smarthome.service.TimeUnit
...
class FiltersActivity : AppCompatActivity(), View.OnClickListener, AdapterView.OnItemSelectedListener {
    ...
    class Data(var mResult: Int, var mDateStart: LocalDate?, var mDateOffset: Int,
               var mDateOffsetUnit: TimeUnit, var mPeriod: Int, var mPeriodUnit: TimeUnit
    ) : Parcelable {
        ...
        fun getDateOffset(): DateOffset? {
            return if (mDateOffset == 0) { null } else {
                DateOffset(mDateOffset, mDateOffsetUnit)
            }
        }
    }
